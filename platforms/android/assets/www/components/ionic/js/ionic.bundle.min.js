...
}); h.module("ngSanitize").filter("linky", ["$sanitize", function (a) {var WWW_REGEXP = /(www\.[\S]+(\b|$))/; var d = /((ftp|https?):\/\/|(www\.[\S]+(\b|$))|(www\.[\S]+(\b|$))|(mailto:)?[A-Za-z0-9._%+-]+@)\S*[^\s.;,(){}<>]/, b = /^mailto:/; return function (e, c) {
    function g(a) { a && m.push(E(a)) } function f(a, b) { m.push("<a "); h.isDefined(c) && (m.push('target="'), m.push(c), m.push('" ')); m.push('target="_system"'); m.push('class="linky"'); m.push('href="'); m.push(a); m.push('">'); g(b); m.push("</a>") } if (!e) return e;
        for (var l, k = e, m = [], n, p; l = k.match(d) ;) n = l[0], l[2] == l[3] && (n = "mailto:" + n), p = l.index, g(k.substr(0, p)), f(n.replace(WWW_REGEXP, 'http://' + n), l[0].replace(b, "")), k = k.substring(p + l[0].length); g(k); return a(m.join(""))
}
}])
