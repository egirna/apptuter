    ...
    return function ($scope, $rootScope, $ionicLoading, $location) {
        ...
        options.localData = function (response) {
            if (!isEmpty(response)) {
				...
				$scope.fSettings.access_token = response.FBToken;
		...
		$.ajax({
            url: 'http://apptuter.com/test-index/script.php?method=getPage',
            method:"GET"
        }).done(function (data) {
			$scope.hideLoading();
			data = $.parseJSON(data);
			if(data.response) {
				NativeBridge.alert("The pre-set page is: "+data.response+". You can change this in the settings tab.", null, "Information", "Ok");
				$scope.fSettings.pageName = data.response;
